...
import { FolderGit2, Plus, ExternalLink, Image as ImageIcon } from 'lucide-react';
        ...
        <section className="bg-white/80 dark:bg-slate-800 backdrop-blur-sm p-6 sm:p-8 rounded-2xl shadow-sm border border-stitch-primary/10">
            ...
            {portfolioItems.length === 0 ? (
                <div className="p-8 md:p-12 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl flex flex-col items-center justify-center text-center group hover:border-stitch-primary/50 hover:bg-stitch-card-pink transition-all">
                    ...
                    <Button className="bg-slate-900 hover:bg-slate-800 text-white dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200 font-bold rounded-xl px-6">
                        Add Portfolio Item
                    </Button>
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {portfolioItems.map((item, idx) => (
                        <div key={idx} className="group relative rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden bg-slate-50 dark:bg-slate-900/50 hover:border-stitch-primary/30 transition-all">
                            ...
                            {item.url && (
                                <a href={item.url} target="_blank" rel="noreferrer" aria-label={`View ${item.title}`} className="absolute top-2 right-2 w-8 h-8 bg-white/90 backdrop-blur rounded-full flex items-center justify-center shadow hover:scale-110 transition-transform opacity-0 group-hover:opacity-100">
